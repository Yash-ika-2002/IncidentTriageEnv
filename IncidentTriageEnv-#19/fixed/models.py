...
class RewardBreakdown(BaseModel):
    ...
    resolution_quality: float = Field(
        0.02, ge=0.02, le=0.15,
        description="+0.15 for a well-formed resolution message"
    )
    step_penalty: float = Field(
        default=-0.01,
        description="-0.1 per step beyond threshold; -0.01 when no penalty applies"
    )
    repeat_penalty: float = Field(
        default=-0.01,
        description="-0.05 per repeated action; -0.01 when no penalty applies"
    )
...
class IncidentReward(BaseModel):
    ...
    total: float = Field(
        ..., gt=0.0, lt=1.0,
        description="Clamped total reward for this step, strictly between 0 and 1"
    )
